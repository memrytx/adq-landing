...
export function Hero() {
  ...
  return <section className={`${s.hero} ${s.glowTop}`} id="top"><GlowAccent asset="bridge" className={s.heroGlowAsset} parallax={false} /><div className={s.heroCopy}><h1><img className={s.heroTitleLogo} src={adQuantumLogo} alt="AdQuantum" /></h1><p>We make creatives for top publishers<br />all over the world</p></div><div className={s.heroRail}><div className={s.heroTrack}>{loop.map((src, i) => <img src={src} alt="" key={`${src}-${i}`} />)}</div></div><a className={s.gradientButton} href="https://www.adquantum.com/" target="_blank" rel="noreferrer"><span className={s.heroCtaDesktop}>adquantum.design</span><span className={s.heroCtaMobile}>visit our site</span></a><span className={s.heroLogoMobile}><Logo /></span></section>
}
